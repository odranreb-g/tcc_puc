...
build_and_load_images:
	...
	docker build . -t tcc_partner_routes_api:0.0.8 && \
	kind load docker-image tcc_partner_routes_api:0.0.8 && \
	cd -

	cd projects/legacy_system && \
	docker build . -t tcc_legacy_system:0.0.6 && \
	kind load docker-image tcc_legacy_system:0.0.6 && \
	cd -
	...
	docker build . -t tcc_zpl_generate_service:0.0.3 && \
	kind load docker-image tcc_zpl_generate_service:0.0.3 && \
	cd -